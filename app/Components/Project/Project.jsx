import HTML from "../../../public/HTML.png";
import ReactLogo from "../../../public/React.png";
import Tailwind from "../../../public/Tailwind.png";
import { UilGlobe, UilGithub, UilLocationArrow } from '@iconscout/react-unicons';

import Image from "next/image";
export default function Project() {
  return (
    <div className="container px-10 py-8 mx-auto max-w-[480px] sm:max-w-[640px]">
      <h1 className="text-whiteColor text-center text-xl font-medium mb-8
      sm:text-2xl">Project I have been Made</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
        <div className="bg-Black3 p-7 max-w-[330px] mx-auto rounded-3xl shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] shadow-Black2">
          <div className="flex justify-between flex-col gap-16">
            <div className=" ">
              <div className="flex justify-start items-center gap-4 mb-4">
                <Image src={HTML} alt='HTML' className="w-[10%]" />
                <Image src={ReactLogo} alt='React' className="w-[12%]" />
                <Image src={Tailwind} alt='Tailwind' className="w-[12%]" />
              </div>
              <div className="title text-whiteColor font-medium text-3xl mb-2">
                <h1>Eventoy <br />
                  Landing Page</h1>
              </div>
              <div className="subtitle text-Gray font-normal text-md">
                <p>Find Any Perfect Venue For Whereever Life Takes You!</p>
              </div>
            </div>
            <div className="flex justify-center items-center text-whiteColor text-sm gap-2">
              <div className="flex justify-center items-center gap-2 ">
                <UilGlobe />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Website</p>
                  <UilLocationArrow />
                </div>
              </div>
              <div className="flex justify-center items-center gap-2">
                <UilGithub />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Github</p>
                  <UilLocationArrow />
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-Black3 p-7 max-w-[330px] mx-auto rounded-3xl shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] shadow-Black2">
          <div className="flex justify-between flex-col gap-16">
            <div className=" ">
              <div className="flex justify-start items-center gap-4 mb-4">
                <Image src={HTML} alt='HTML' className="w-[10%]" />
                <Image src={ReactLogo} alt='React' className="w-[12%]" />
                <Image src={Tailwind} alt='Tailwind' className="w-[12%]" />
              </div>
              <div className="title text-whiteColor font-medium text-3xl mb-2">
                <h1>Eventoy <br />
                  Landing Page</h1>
              </div>
              <div className="subtitle text-Gray font-normal text-md">
                <p>Find Any Perfect Venue For Whereever Life Takes You!</p>
              </div>
            </div>
            <div className="flex justify-center items-center text-whiteColor text-sm gap-2">
              <div className="flex justify-center items-center gap-2 ">
                <UilGlobe />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Website</p>
                  <UilLocationArrow />
                </div>
              </div>
              <div className="flex justify-center items-center gap-2">
                <UilGithub />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Github</p>
                  <UilLocationArrow />

                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-Black3 p-7 mx-auto max-w-[330px] rounded-3xl shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] shadow-Black2">
          <div className="flex justify-between flex-col gap-16">
            <div className=" ">
              <div className="flex justify-start items-center gap-4 mb-4">
                <Image src={HTML} alt='HTML' className="w-[10%]" />
                <Image src={ReactLogo} alt='React' className="w-[12%]" />
                <Image src={Tailwind} alt='Tailwind' className="w-[12%]" />
              </div>
              <div className="title text-whiteColor font-medium text-3xl mb-2">
                <h1>Eventoy <br />
                  Landing Page</h1>
              </div>
              <div className="subtitle text-Gray font-normal text-md">
                <p>Find Any Perfect Venue For Whereever Life Takes You!</p>
              </div>
            </div>
            <div className="flex justify-center items-center text-whiteColor text-sm gap-2">
              <div className="flex justify-center items-center gap-2 ">
                <UilGlobe />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Website</p>
                  <UilLocationArrow />
                </div>
              </div>
              <div className="flex justify-center items-center gap-2">
                <UilGithub />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Github</p>
                  <UilLocationArrow />

                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="bg-Black3 p-7 mx-auto max-w-[330px] rounded-3xl shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] shadow-Black2">
          <div className="flex justify-between flex-col gap-16">
            <div className=" ">
              <div className="flex justify-start items-center gap-4 mb-4">
                <Image src={HTML} alt='HTML' className="w-[10%]" />
                <Image src={ReactLogo} alt='React' className="w-[12%]" />
                <Image src={Tailwind} alt='Tailwind' className="w-[12%]" />
              </div>
              <div className="title text-whiteColor font-medium text-3xl mb-2">
                <h1>Eventoy <br />
                  Landing Page</h1>
              </div>
              <div className="subtitle text-Gray font-normal text-md">
                <p>Find Any Perfect Venue For Whereever Life Takes You!</p>
              </div>
            </div>
            <div className="flex justify-center gap-2 items-center text-whiteColor text-sm">
              <div className="flex justify-center items-center gap-2 ">
                <UilGlobe />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Website</p>
                  <UilLocationArrow />
                </div>
              </div>
              <div className="flex justify-center items-center gap-2">
                <UilGithub />
                <div className="flex justify-center items-center gap-1 flex-row">
                  <p>Github</p>
                  <UilLocationArrow />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
